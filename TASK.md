...
- [x] 역할 선택 UI를 단일 드롭다운 형식으로 조정
- [x] 직원 메뉴를 상단 탭형 카테고리 구조로 조정
- [x] 직원 메뉴를 헤더 바로 아래 상단 영역으로 재배치

## Phase 6. 기본 등록 기능 구현

- [x] `BasicOrderForm.tsx` 기본 구조 구현
- [x] 빠른 문장 입력 영역 구현
- [x] `자동 분리` 버튼과 로컬 파서 연결
- [x] 거래처 입력 및 기존 거래처 자동완성 구현
- [x] 거래처 선택 시 최근 현장 추천 UI 구현
- [x] 제품 및 규격 입력 영역 구현
- [x] 작업 정보 입력 영역 구현
- [x] 평수 자동 계산 표시 구현
- [x] 필수값 검증 및 오류 메시지 구현
- [x] 주문 저장 기능 구현
- [x] `직전 주문 불러오기` 기능 연결
- [x] `즐겨찾기로 저장` 기능 연결

## Phase 6-1. 기존 데이터 업로드 화면 구현

- [x] `파일 업로드` 별도 카테고리 추가
- [x] `ExistingDataUploadPanel.tsx` 구현
- [x] 파일 선택 UI 및 안내 문구 구현
- [x] 엑셀/CSV 미리보기 및 일괄 등록 구현

## Phase 7. 빠른 등록 기능 구현

- [x] `QuickRegister.tsx` 기본 구조 구현
- [x] 거래처 검색 UI 구현
- [x] 거래처 선택 시 최근 현장 추천 구현
- [x] 거래처 + 현장 기준 자주 쓰는 품목 카드 구현
- [x] 카드 선택 후 수량 입력 영역 구현
- [x] 빠른 등록 저장 기능 구현
- [x] `직전 주문 불러오기` 기능 연결

## Phase 8. 즐겨찾기 기능 구현

- [x] `FavoritesPanel.tsx` 구현
- [x] 즐겨찾기 목록 조회 구현
- [x] 즐겨찾기 카드 UI 구현
- [x] `불러오기` 기능 구현
- [x] `바로 등록` 기능 구현
- [x] 수량 없는 즐겨찾기용 입력 처리 구현
...
- [x] 통합 검색 기능 구현
- [x] 상태 필터 구현
- [x] 행 클릭 상세 보기 또는 수정 모달 구현
- [x] 상태 변경 버튼 구현

## Phase 10. 확인필요 목록 구현

- [x] `NeedsCheckList.tsx` 구현
- [x] 확인필요 조건 필터링 로직 구현
- [x] 누락 항목이 드러나는 목록 UI 구현

## Phase 11. 메인 페이지 연결 및 마감 정리

- [x] `app/page.tsx`에 전체 화면 조립
- [x] `app/layout.tsx` 메타데이터 및 공통 구조 정리
- [x] `app/globals.css` 업무용 대시보드 스타일 완성
- [x] README 설치 및 배포 가이드 완성
- [x] TODO 주석 추가
  - 추후 AI API 연결
  - 추후 Auth/권한 연결
  - 추후 팀장/대표 대시보드 구현
...
- [ ] Vercel 배포 성공
- [ ] 모바일 반응형 정상 동작
- [x] 핵심 기능 전체 플로우 테스트 완료
- [x] Supabase 데이터 생성/조회/수정 정상 동작
- [x] README.md 배포 안내 작성 완료
