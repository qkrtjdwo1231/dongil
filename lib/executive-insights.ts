  ...
  return groupOrdersByDimension(orders, "customer")
    ...
    .map((group) => {
      ...
      if (family && family.label === "로이/코팅") {
        ...
        label = "대형 규격 고객";
        reason = `건당 평균 평수가 ${avgArea.toFixed(2)}로 커서 포장 부담이 높습니다.`;
      } else if (activeMonths <= 2) {
        label = "프로젝트성 고객";
        reason = `활동 월 수가 ${activeMonths}개월로 짧아 특정 프로젝트 집중 가능성이 있습니다.`;
...
export function buildExecutiveInsightReport({
  ...
  const rowsMissingDimensions = snapshot?.rowsMissingDimensions ?? 0;
  const rowsMissingSite = snapshot?.rowsMissingSite ?? 0;
  const peakMonthLabel = snapshot?.peakMonthLabel ?? null;
  const peakHourLabel = snapshot?.peakHourLabel ?? null;
  ...
  const changes = [
    ...
    peakMonthLabel
      ? `최근 업로드 기준 피크 월은 ${peakMonthLabel}, 피크 시간대는 ${peakHourLabel ?? "집계 없음"}입니다.`
  ...
  const quality = [
    ...
    snapshot
      ? `최근 업로드 기준 규격 누락 ${rowsMissingDimensions.toLocaleString()}건, 현장 미입력 ${rowsMissingSite.toLocaleString()}건이 확인됩니다.`
      : "최근 업로드 요약 데이터가 아직 없습니다."
  ];

  const actions: string[] = [];
  if (topCustomerShare >= 50) {
    actions.push("상위 거래처 집중도가 높으므로 주요 거래처 의존 리스크를 함께 점검해 주세요.");
  }
  if (topItemShare >= 50) {
    actions.push("상위 품목 쏠림이 높으므로 자재 준비와 생산 계획을 상위 품목 중심으로 재점검해 주세요.");
  }
  if (siteMissingRate >= 5) {
    actions.push("현장 누락이 분석 신뢰도를 낮추므로 업로드 전 데이터 정제 규칙을 강화해 주세요.");
  }
  if (!actions.length) {
    actions.push("현재 구간은 구조적 리스크보다 추세 변화와 상위 거래처 흐름 모니터링이 더 중요합니다.");
  }
