...
- 상단 헤더 우측에는 현재 선택된 역할만 보이는 단일 버튼형 역할 선택 UI를 제공한다.
- 역할 선택 버튼 클릭 시 `직원`, `팀장`, `대표` 목록이 펼쳐지는 드롭다운 형식으로 동작한다.
...
- 즐겨찾기
- 체크리스트
- 직원 메뉴는 카드형이 아니라 화면 상단 카테고리 탭 형태로 표시한다.
- 직원 메뉴는 헤더 바로 아래 상단 영역에 배치한다.

### 2.3 파일 업로드

- 직원 화면에서 `파일 업로드`를 별도 카테고리로 제공한다.
- 이 화면은 기존 엑셀 데이터를 이관하기 위한 전용 공간이다.
- 1차 MVP에서는 실제 엑셀 파싱이나 DB 반영을 구현하지 않는다.
- 업로드한 파일은 미리보기 후 유효 행만 주문 데이터로 저장할 수 있어야 한다.
- 거래처, 품명, 수량이 없는 행은 검토 필요로 분류한다.
...
- 직원 기능 메뉴는 화면 최상단에서 빠르게 이동 가능한 탭형 카테고리 구조를 사용한다.
- 직원 기능 메뉴는 헤더 바로 아래쪽에 붙는 상단 내비게이션 형태로 배치한다.
- 역할 표시는 우측 상단의 단일 버튼에 현재 역할명만 노출하고, 펼침 시 역할 목록이 보이도록 한다.
...
- 파일 업로드는 기본 등록 내부가 아니라 별도 카테고리 화면으로 분리한다.
