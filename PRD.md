...
- 기본 등록 화면 상단에 기존 엑셀 데이터를 위한 업로드 공간을 제공한다.
- 1차 MVP에서는 실제 엑셀 파싱이나 DB 반영을 구현하지 않는다.
- 현재 단계에서는 파일 선택 UI와 안내 문구만 제공한다.
- 안내 문구에는 기존 데이터 이관용 임시 공간이며, 추후 업로드 처리 로직을 연결할 예정임을 표시한다.
...
- 기본 등록 상단에는 기존 데이터 이관을 위한 엑셀 업로드 공간을 함께 배치한다.
